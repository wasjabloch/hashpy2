...
def print_config_tipps():
    ...
    print("    hypocenter, location error and phase information, or")
    print("    HypoDD phase file.")
    print("RESULTS:")
    ...
    print("")
    print("FORMAT:")
    print("    Format of the waveform database.")
    print("    FUB2018: Format at Ferie Universitaet Berlin 2018")
    print("    UBC2022: Format at University of British Columbia 2022")
    print("")
    print("")
    print("VELOCITIES:")
...
def get_event_hypodd(ID, phasefile):
    """
    Retrun obspy.Event near time t0 from list of hypfiles
    """
    events = read_events(phasefile)
    for ev in events:
        if ID in ev.resource_id.id:
            timestamp = ev.origins[0].time
            return ev, timestamp
    msg = f"ID not found in catalog: {ID}"
...
def find_ID_from_t0(t0, sIDs):
    ...
    t0 = float(t0)
    ids = {
        float(UTCDateTime.strptime(thisID, "%y%m%d%H%M%S")): thisID for thisID in sIDs
    }
    dtmin = np.inf
    for t in sorted(ids):
        dt = abs(t - t0)
...
def make_traces_nll(timestamp, wvdir):
    """
    Read seismic traces stored in directory structre YYYY/YYMMDDhhmmss
    """
    filterwarnings(
        "ignore", category=UserWarning, message="Record contains a fractional second"
    )
...
def make_traces_hypodd(timestamp, wvdir):
    """
    Read seismic traces stored in directory structre YYMMDDhhmmss
    """
    filterwarnings(
        "ignore", category=UserWarning, message="Record contains a fractional second"
    )
    folder = timestamp.strftime("%y%m%d") + "0000"
    wavedir = wvdir + folder
    traces = read(wavedir + "/*/*[BH]H*")
    return traces
...
def dist2stations_nll(stations, event, filename):
    """
...
def dist2stations_hypodd(stations, event, filename):
    """
    Compute dictionary that holds event - station distances
    station -> distance (km)
    """
...
def PickRequest(traces, station, event, dist):
    logging.debug("This is PickRequest")
...
def AutoPicker(traces, station, event, dist, component="?HZ"):
    logging.debug("This is AutoPicker")
    starttime, endtime = GetTimeWindow(event, station)
    tr = (
        traces.select(station=station, channel=component)
...
def PolarityPicker(picktime, trace, gs):
    logging.debug("This is PolarityPicker")
    poltrace = trace.copy()

    pol, poltime = PolarityStd(
        picktime, poltrace.filter("highpass", freq=1.0), gs[3, 1]
    )
    polhint = "Std"
    poltrace = trace.copy()
    if pol in ["+", "-"]:
...
def PolarityGradient(picktime, trace, gs, color="black"):
    logging.debug("This is PolarityGradient")
    timewindow = 0.5
    poltrace = trace.copy()
            ...
            if abs(d2 - d1) < abs(d3 - d2):
                ...
                if pol in ["+", "-"]:
                    try:
                        ...
                        continue
                    if np.sign(dp - dpm1) != np.sign(dpm1 - dpm2) and np.sign(
                        dpp2 - dpp1
                    ) != np.sign(dpp1 - dp):
                        continue
                    else:
    ...
    # gradient picker, the gradient has to be stable for few more samples
        ...
        for ii in range(len(tr.data) - 2 - checkLength):
            ...
            if np.sign(d2 - d1) != np.sign(d3 - d2):
                nn = 0
                while np.sign(d2 - d1) != (
                    np.sign(tr.data[ii + 2 + nn] - tr.data[ii + 1 + nn])
                ):
                    nn += 1
                    if nn > checkLength:
                        poltime = tr.times()[ii + 1]
...
def PolarityStd(picktime, trace, gs, color="black"):
    logging.debug("This is PolarityStd")
    ax0 = plt.subplot(gs)
    ...
    signalstart = 0.5
    signalend = 1.5
    stdSigtr = poltrace.slice(
        starttime=picktime + signalstart, endtime=picktime + signalend
    )
    ...
    poltrace = trace.copy()
    tr1 = poltrace.slice(
        starttime=picktime - timebeforepick, endtime=picktime + timewindow
    )
...
def AskForPolarity(traces, station, event, pick, distance):
    ...
    logging.debug("This is AskForPolarity")
    fig, pol = ShowPolarity(traces, station, event, pick, distance)
    logging.debug("This was ShowPolarity")
    fig.show()
...
def ShowPolarity(traces, station, event, pick, distance, component="?HZ"):
    logging.debug("This is ShowPolarity")
    ...
    ax1.set_title(
        "Raw trace {:}.{:}\nDistance: {:.0f} km".format(station, component, distance)
    )
    ...
    plt.text(
        picktime - startzoom + poltime + 0.05,
        0.9 * (max(trzoom[0].data)),
        "%s" % (polhint),
        color="blue",
    )
...
def GetTimeWindow(event, station, distance=0):
    logging.debug("This is GetTimeWindow")
...
def AskIfPolarityIsGood(traces, station, event, pick, distance, pol):
    logging.debug("This is AskIfPolarityIsGood")
    msg = (
        "\nThis is station "
        + station
        + ".\n"
...
def get_spratio(traces, station, pick, dist):
    ...
    tr = (
        traces.select(station=station, channel="[BH]H?")
        .slice(starttime=starttime - 10, endtime=endtime + 10)
        .detrend("demean")
        .integrate()
        .detrend("linear")
        .slice(starttime=starttime, endtime=endtime)
    ...
    ax1 = plt.subplot(311)
    ax1.plot(
        tr[0].times(reftime=picktime)[int(cuttime * df) :] - cuttime,
        sumtrace[int(cuttime * df) :],
        color="black",
    )
    ...
    ax2.plot(trfil[0].times(reftime=picktime), sumtracefil, color="black")
    ...
    ax3.plot(
        trfil[0].times(reftime=picktime), trfil[0].data, linewidth=0.5, color="black"
    )
    ax3.plot(
        trfil[0].times(reftime=picktime),
        trfil[1].data,
        linewidth=0.5,
        color="steelblue",
    )
    ax3.plot(
        trfil[0].times(reftime=picktime), trfil[2].data, linewidth=0.5, color="purple"
    )
    ax3.plot(0, 0, marker="|")
...
def ask_for_amplitudes(p="x", s="x"):
    logging.debug("This is ask_for_amplitudes")
    """
...
def AskIfSPRatioIsGood(traces, station, pick, dist, soverp):
    logging.debug("This is AskIfSPRatioIsGood")
...
def create_HASH_runfile(hashfile, polfile, params, ID):
    ...
    while True:
        print(
            (
                "(ENTER) confirm {:.0f} km maximum source receiver distance, "
                + "or type"
            ).format(delmax)
        )
...
def WriteMechanismToResultfile(event, strike, dip, rake, quality, nobs, fn):
    lon, lat, dep, mag= (
        event.origins[0].longitude,
        event.origins[0].latitude,
        event.origins[0].depth * 1e-3,
        event.magnitudes[0].mag,
    )
    buf = "#Lon        Lat       Depth  Strike Dip Rake Magn  RMS Nobs\n"
    buf += "{:9.4f} {:9.4f} {:6.2f}   {:3d} {:3d}  {:3d} {:5.2f} {:3d}   {:3d} \n".format(
        lon, lat, dep, strike, dip, rake, mag, quality, nobs
    )
...
def PlotMechanismHyp(resultdir, hypfile, ID):
    allfpsf = resultdir + ID + ".all.fps"
    angf = resultdir + ID + ".rays"
    plotfile = resultdir + ID + ".ps"
    cmd = "plot_mechanism.sh {:} {:} {:} {:}".format(hypfile, allfpsf, angf, plotfile)
    out, _, _ = runBash(cmd)  # run plotting script
    logging.info("The plotting script returned:\n" + out.decode("utf-8"))
    _, _, _ = runBash("gv " + plotfile)  # run gv


def PlotMechanismRes(resultfile, resultdir, ID):
    allfpsf = resultdir + ID + ".all.fps"
    angf = resultdir + ID + ".rays"
    plotfile = resultdir + ID + ".ps"
    cmd = "./plot_mechanism2.sh {:} {:} {:} {:}".format(resultfile, allfpsf, angf, plotfile)
    out, _, _ = runBash(cmd)  # run plotting script
    _, _, _ = runBash("gv " + plotfile)  # run gv